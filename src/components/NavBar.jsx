            ...
            <div className="container flex items-center justify-between">
                    <a href="#hero" className="text-2xl font-bold text-primary">
                        <span className="realtive z-10">
                            <span className="text-glow text-foreground"> JakobNieß</span>Portfolio
                        </span>
                    </a>
                    ...
                    <div className="hidden md:flex gap-8 space-x-8">
                        {navItems.map((item,key) => (
                            <a key={key} href={item.href} className="text-secondary hover:text-primary transition-color duration-300 ">{item.name}</a>
                        ))}

                    </div>
                    {/*Mobile */}

                        <button className="md:hidden p2 z-50" 
